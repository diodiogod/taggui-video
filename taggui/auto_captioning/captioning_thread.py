...
class CaptioningThread(ModelThread):
    ...
    def load_model(self):
        ...
        if structured_output:
            generation_parameters = self.caption_settings.get(
                'generation_parameters', {}
            )
            generation_parameters['max_new_tokens'] = max(
                2048,
                int(generation_parameters.get('max_new_tokens', 0) or 0),
            )
            self.caption_settings['api_max_tokens'] = max(
                2048,
                int(self.caption_settings.get('api_max_tokens', 0) or 0),
            )
        ...
        if self.caption_settings.get('output_format') == 'Ideogram 4 JSON':
            self.model.caption_start = ''
            self.model.remove_tag_separators = False
            self.model.remove_new_lines = False
            self.model.bad_words_string = ''
            self.model.forced_words_string = ''
    ...
    def get_model_inputs(self, image: Image):
        if self.caption_settings.get('output_format') == 'Ideogram 4 JSON':
            ...
            self._ideogram_seed_elements = []
        crop = (
            False
            if self.caption_settings.get('output_format') == 'Ideogram 4 JSON'
            else self.caption_settings['limit_to_crop']
        )
    ...
    def generate_output(self, image_index, image: Image, image_prompt: str | None, model_inputs) -> str:
        ...
        if self.caption_settings.get('output_format') == 'Ideogram 4 JSON':
            ...
            try:
                ...
                return (
                    f'ERROR: Ideogram 4 JSON was rejected for '
                    f'{image.path.name}: {exc}\n\nRaw model output:\n'
                    f'{console_output_caption}'
                )
            structured = preserve_seed_bboxes(
                structured,
                self._ideogram_seed_elements,
            )
            save_ideogram_caption(image.path, structured, pretty=True)
            self.structured_caption_generated.emit(image_index, structured)
            return structured.to_json(pretty=True)
